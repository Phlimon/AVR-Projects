...
void main(void)
{
	...
	u8 Local_u8String[] = "Phlimonfahim@gmail.com"; //Max 20 Char
...
//	LCD_u8WriteString(Local_u8String, Local_u8StringSize);
	...
	while(1)
	{
		Local_u8Move2 = LCD_OV_LINE_2 - Local_u8StringSize;

		for(Local_u8Move = 0; Local_u8Move < LCD_MAX_SCROLL_LINE_2; Local_u8Move++)
		{
			//if(Local_u8Move >= 2)
			if(Local_u8Move >= (LCD_LINE_NB_OF_DIGITS - Local_u8StringSize))
			{
				...
				LCD_u8GoToXY(Local_u8Move2, 1);
				LCD_u8WriteString(Local_u8String, Local_u8StringSize);

				LCD_u8WriteINT(Local_u8Move, 0, 0);
				LCD_u8WriteINT(Local_u8Move2, 5, 0);
				LCD_u8WriteINT(Local_u8StringSize, 10, 0);
